...
public class ChadmiumCompressorMenu extends AbstractContainerMenu {

    public final ChadmiumCompressorBlockEntity entity;
    private final Level level;
    private final ContainerData data;

    public ChadmiumCompressorMenu(int id, Inventory inv, FriendlyByteBuf extra) {
    ...
    public ChadmiumCompressorMenu(int id, Inventory inv, @Nullable BlockEntity entity, ContainerData data) {
        super(GigaMenus.CHADMIUM_COMPRESSOR.get(), id);
        checkContainerSize(inv, 17);
        this.entity = (ChadmiumCompressorBlockEntity) entity;
        ...
        this.entity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(h -> {
            addSlot(new SlotItemHandler(h, 0, 136, 44 + Y_OFFSET));
            for(int i = 0; i < 4; i++)
                for(int j = 0; j < 4; j++)
    ...
    public boolean stillValid(@Nonnull Player player) {
        return stillValid(ContainerLevelAccess.create(level, entity.getBlockPos()), player, GigaBlocks.CHADMIUM_COMPRESSOR.get());
    }
